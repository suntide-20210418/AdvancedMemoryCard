...
public class ModGuiHandler implements IGuiHandler {
    ...
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == CONFIG_GUI_ID) {
            ...
            return new ConfigModeMenu(player.inventory, player.getHeldItemMainhand().isEmpty() ? net.minecraft.util.EnumHand.MAIN_HAND : net.minecraft.util.EnumHand.values()[x], manager);
        } else if (ID == COPY_GUI_ID) {
            return new CopyModeMenu(player.inventory, net.minecraft.util.EnumHand.values()[x]);
        }
        return null;
    }
