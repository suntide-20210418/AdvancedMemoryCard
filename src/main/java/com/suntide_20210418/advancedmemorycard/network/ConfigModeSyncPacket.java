...
public class ConfigModeSyncPacket implements IMessage {
    ...
    private static P2PPosition readP2PPosition(ByteBuf buf) {
        ...
        return new P2PPosition(pos, appeng.api.util.AEPartLocation.fromFacing(direction), dimension);
    }
    ...
    private static P2PInfo readP2PInfo(ByteBuf buf) {
        ...
        BlockPos position = BlockPos.fromLong(buf.readLong());
        net.minecraft.util.EnumFacing direction = net.minecraft.util.EnumFacing.values()[buf.readByte()];
        return new P2PInfo(isActive, isOutput, isConnected, isMEP2P, isPendingBind, channel, maxChannel,
                frequency, name, p2pType, dimension, dimensionId, position, appeng.api.util.AEPartLocation.fromFacing(direction));
    }
